...
def rsi(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    rs = ic.avg_gain(df, 5) / ic.avg_loss(df, 5).replace(0, np.nan)
    df["RSI"] = 100 - (100 / (1 + rs))
...
def roc(df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    """
    prev_close_n = df.groupby("Ticker")["Close"].transform(lambda s: s.shift(5)).replace(0, np.nan)
    df["ROC"] = (df["Close"] / prev_close_n) - 1

    return df

def stoch_perc_k(df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    """
    rolling_high = df.groupby("Ticker")["High"].transform(lambda s: s.rolling(5).max())
    rolling_low = df.groupby("Ticker")["Low"].transform(lambda s: s.rolling(5).min())
    denom = (rolling_high - rolling_low).replace(0, np.nan)
    df["StochPercK"] = (df["Close"] - rolling_low) / denom
